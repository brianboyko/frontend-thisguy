...
function chargeCard(token)
{
  ...
  $.post('https://thisguy-donation.herokuapp.com/charge', params,
    // Success
    function () {
      var date = new Date();
...
var stripeHandler = StripeCheckout.configure({
  key:             stripe_pubkey,
  name:            'Donating to Send This Guy',
...
var changeMonies = function(amount) {
  document.getElementById('monies').value = amount;
};

function makeChangeMonies(amount) { 
  return function() { 
    return changeMonies(amount); 
  }; 
}; 

[10, 25, 50, 100, 250, 500, 1000, 2500, 5000].forEach(function(amount) { document.getElementById("donate" + amount).addEventListener("click", makeChangeMonies(amount), false); });
